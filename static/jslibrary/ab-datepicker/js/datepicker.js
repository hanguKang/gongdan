...
(function(factory){
	...
	Datepicker.prototype.show = function() {
		...
		if (roomAfter < calendarHeight && roomAfter < roomBefore) {
			...
			// console.log(groupOffsetLeft , parentPaddingLeft , this.$group.parents('.lpopwarp').offset().left)
			if (this.$group.parents('.lpopwarp').length) {
				this.$calendar.css({
					top: (groupOffsetTop - calendarHeight - this.$group.parents('.lpopwarp').offset().top) + 'px',
					left: (groupOffsetLeft + parentPaddingLeft - this.$group.parents('.lpopwarp').find('>div').offset().left) + 'px'
				});
			}
			...
			if (this.$group.parents('.lpopwarp').length) {
				this.$calendar.css({
					top: (groupOffsetTop + groupHeight - this.$group.parents('.lpopwarp').offset().top) + 'px',
					left: (groupOffsetLeft + parentPaddingLeft - this.$group.parents('.lpopwarp').find('>div').offset().left) + 'px'
				});
			}
		}
